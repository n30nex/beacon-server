...
func TestAtlasBoundsUsesAggregatesOnlyForCompleteHours(t *testing.T) {
	since := time.Date(2026, 7, 10, 1, 16, 24, 0, time.UTC)
	until := since.Add(24 * time.Hour)
	bounds := atlasBounds(since, until)
	if want := time.Date(2026, 7, 10, 2, 0, 0, 0, time.UTC); !bounds.fullStart.Equal(want) {
		t.Fatalf("fullStart = %s, want %s", bounds.fullStart, want)
	}
	if want := time.Date(2026, 7, 11, 1, 0, 0, 0, time.UTC); !bounds.fullEnd.Equal(want) {
		t.Fatalf("fullEnd = %s, want %s", bounds.fullEnd, want)
	}
	if got := bounds.fullEnd.Sub(bounds.fullStart) / time.Hour; got != 23 {
		t.Fatalf("complete aggregate hours = %d, want 23", got)
	}
}
