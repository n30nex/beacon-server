...
func main() {
	...
	if redisAddr := os.Getenv("REDIS_ADDR"); redisAddr != "" {
		...
		if err := redisClient.Ping(ctx); err != nil {
			...
			cacheStatus = "ok"
			cacheBackend = redisAddr
			log.Printf("cache: Redis connected at %s (atlas=%s stats=%s reference=%s nodes=%s observers=%s)",
				redisAddr, ttls.Atlas, ttls.Stats, ttls.Reference, ttls.Nodes, ttls.Observers)
		}
	}
